...
async def leave_task(sid, c, ci):
    try:
        ...
        if str(group_id).lstrip('-').isdigit() and not str(group_id).startswith('-100'):
            group_id = f"-100{abs(group_id)}"
        # Fire-and-forget leave — don't await, return instantly
...
async def setup_handlers(client):
    ...
    async def handle_message(e):
        ...
        if cmd == 'start':
            ...
            try:
                msg = await e.reply("⏳ Exporting sessions...")
                ...
                await msg.edit(f"✅ Exporting {total} sessions (sequence: {sorted_sids[0]}→{sorted_sids[-1]})...")
                await client.send_file(
                    e.chat_id,
                    zip_path,
                    caption=f"📦 **Exported Sessions**\n\n✅ Total: {total}\n🔢 Sequence: S{sorted_sids[0]} → S{sorted_sids[-1]}\n📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    reply_to=e.id
                )
                os.remove(zip_path)
                await msg.delete()
