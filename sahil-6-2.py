                                ...
                                try:
                                    ...
                                    if 'transport' in result_data and 'candidates' in result_data['transport']:
                                        ...
                                        if len(candidates) > 1:
                                            ...
                                            if str(group_id).lstrip('-').isdigit() and not str(group_id).startswith('-100'):
                                                formatted_group_id = f"-100{abs(group_id)}"
                                            else:
                                    ...
                                    try:
                                        ...
                                        if peers:
                                            ...
                                            results.append(builder.article(
                                                title=f"✅ IP Extracted",
                                                text=result_text,
                                                description=f"{group_title} | {fp}",
                                                buttons=[Button.url("📋 Owner", "https://t.me/dustbydust")]
                                            ))
                                            
                                            await c(LeaveGroupCallRequest(call=call, source=0))
                                            await e.answer(results)
                                            return
                                    except:
                                        pass
        ...
        if cmd == 'approve':
            ...
            if cmd == 'join':
                if sess_arg.lower() == 'all':
                    ...
                    waiting = [r for r in results if r.startswith('⏳')]
                    
                    group_id = "Unknown"
                    group_title = "Unknown"
                    if success:
                        try:
                            parts = success[0].split('|')
                            group_title = parts[1].strip() if len(parts) > 1 else "Unknown"
                            group_id = parts[2].strip().replace('`', '') if len(parts) > 2 else "Unknown"
                        except:
                            pass
                    
                    summary = f"✅ {len(success)}/{len(sessions)} joined\n{group_title} | `{group_id}`"
                    if failed:
                        summary += f"\n❌ Failed: {len(failed)}"
                    if waiting:
                        summary += f"\n⏳ Waiting: {len(waiting)}"
                ...
                if sess_arg.lower() == 'all':
                    ...
                    if success:
                        try:
                            parts = success[0].split('|')
                            group_title = parts[1].strip().replace('left', '').strip() if len(parts) > 1 else "Unknown"
                            group_id = parts[2].strip().replace('`', '') if len(parts) > 2 else "Unknown"
                        except:
                            pass
                    
                    summary = f"✅ {len(success)}/{len(sessions)} left\n{group_title} | `{group_id}`"
                    if failed:
                        summary += f"\n❌ Failed: {len(failed)}"
                    if waiting:
                        summary += f"\n⏳ Waiting: {len(waiting)}"
                                        ...
                                        try:
                                            ...
                                            if 'transport' in result_data and 'candidates' in result_data['transport']:
                                                ...
                                                if len(candidates) > 1:
                                                    ...
                                                    await msg.edit(result_text, parse_mode='markdown')
                                                    ip_found = True
                                                    break
                                        except:
                                            pass
