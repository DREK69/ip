...
async def setup_handlers(client):
    ...
    async def inline_handler(e):
        ...
        try:
            ...
            # ✅ FIXED: Proper error handling for data extraction
            try:
                # Try to extract candidates list
                ...
                # Safely extract IP and port with defaults
                ip = data.get("ip", "N/A")
    ...
    async def handle_zip(e):
        ...
        try:
            ...
            # Build existing hashes - only if files exist in sessions folder
            existing_hashes = {}
            if os.path.exists("sessions"):
                for f in os.listdir("sessions"):
                    if f.endswith(".session"):
                        full_path = os.path.join("sessions", f)
            ...
            async def try_load(sf):
                ...
                # Only skip if hash exists AND matches an existing file
                if fh and fh in existing_hashes:
                    async with lock: 
                        skipped += 1
                    return
                
                # File is NEW - move it to sessions folder
                final_path = os.path.join("sessions", sf)
                
                # If file already exists with same name, remove it first
