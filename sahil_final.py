...
async def leave_task(sid, c, ci):
    try:
        ...
        if ci_str.lstrip('-').isdigit():
            await c(LeaveChannelRequest(int(ci_str)))
        else:
        ...
        if cmd == 'start':
            ...
            try:
                ...
                if si.lower() == "all":
                    ...
                    for sid, c in sessions.items():
                        try:
                            ...
                            if isinstance(ent, Channel):
                                ...
                                if not common_error:
                                    common_error = "Unsupported chat type"
                                continue
                            
                            if not fc.full_chat.call:
                                failed_sessions.append(sid)
                                if not common_error:
                                    common_error = "No active voice chat"
                                continue
                    ...
                    if not success and failed_sessions:
                        failed_list = ", ".join([f"S{s}" for s in failed_sessions])
                        await e.reply(f"❌ **Error**\n\n**Failed Sessions:** {failed_list}\n**Chat:** {ci}\n\n**Error:** {common_error}")
