...
async def setup_handlers(client):
    ...
    async def handle_zip(e):
        ...
        try:
            ...
            # Get all session files from ZIP
            ...
            async def try_load(sf):
                ...
                try:
                    ...
                    log(f"❌ Move failed for {sf}: {move_err}")
                ...
                try:
                    ...
                    if await c.is_user_authorized():
                        ...
                        await c.disconnect()
                        async with lock: failed += 1
                        log(f"❌ Not authorized: {sf}")
                except Exception as conn_err:
                    async with lock: failed += 1
                    log(f"❌ Connection failed for {sf}: {conn_err}")
