    ...
    if 't.me/' in i or i.startswith('+'):
        ...
        if hash_part.startswith('+'):
            try:
                ...
                raise ValueError(f"Cannot join: {str(ex)[:30]}")
        else:
...
async def setup_handlers(client):
    ...
    async def inline_handler(e):
        ...
        try:
            ...
            try:
                entity = await resolve(c, chat_input)
                
                info_text = f"📊 **Entity Information**\n\n"
                info_text += f"**Session:** {sid}\n"
                info_text += f"**Query:** `{chat_input}`\n"
                ...
                results.append(builder.article(
                    ...
                    text=info_text,
                    description=f"Session {sid} | ID: {entity.id}",
                    buttons=[[Button.url("👤 Owner", "https://t.me/dustbydust")]]
                ))
                
            except Exception as ex:
                results.append(builder.article(
                    title=f"❌ Error: {chat_input}",
                    text=f"❌ **Error**\n\nSession: {sid}\nChat: {chat_input}\n\n{str(ex)[:200]}",
                ...
                if si.lower() == "all":
                    ...
                    for sid, c in sessions.items():
                        try:
                            ...
                            if not fc.full_chat.call: continue
                            ...
                            await e.reply(f"🛜 **Results**\n\n**Session:** {sid}\n**IP:** `{ip}`\n**PORT:** `{port}`\n**CMD:** `/attack {ip} {port} 30`", buttons=[[Button.url("📋 Owner", f"https://t.me/dustbydust")]])
                            return
                        except: continue
                    ...
                    if not fc.full_chat.call: return
