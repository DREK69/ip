    ...
    if 't.me/' in i or i.startswith('+'):
        ...
        if hash_part.startswith('+'):
            try:
                ...
                # If already participant, get from dialogs
                if "already a participant" in str(e).lower():
                    ...
                    for dialog in dialogs:
                        if hasattr(dialog.entity, 'broadcast') or hasattr(dialog.entity, 'megagroup'):
                            # Return the chat - usually the most recent one is what we need
...
async def getip_task(sid, c, ci, delay=0):
    try:
        ...
        # Get entity - try multiple ways
        entity = None
        try:
            entity = await resolve(c, ci)
        except:
            # If resolve fails, try getting from dialogs
            dialogs = await c.get_dialogs()
            ci_lower = ci.lower()
            for dialog in dialogs:
                if hasattr(dialog.entity, 'username') and dialog.entity.username:
                    if dialog.entity.username.lower() in ci_lower:
                        entity = dialog.entity
                        break
                if hasattr(dialog.entity, 'id'):
                    if str(dialog.entity.id) in ci:
                        entity = dialog.entity
                        break
        
        if not entity:
            return f"❌ Session {sid}: Cannot find chat", None
