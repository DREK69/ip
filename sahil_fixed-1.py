    ...
    async def inline_handler(e):
        ...
        try:
            ...
            try:
                ...
                try:
                    jp = DataJSON(data=json.dumps({"ufrag": "test", "pwd": "test", "fingerprints": [{"hash": "test", "setup": "active", "fingerprint": "test"}], "ssrc": 1}))
                    join_result = await c(JoinGroupCallRequest(call=call, join_as=await c.get_me(), params=jp, muted=True))
...
async def load_existing_sessions():
    ...
    for sf in session_files:
        tasks.append(load_session(sf, next_id))
        next_id += 1
