        ...
        try:
            ...
            try:
                for d in await c.get_dialogs(limit=200):
                    if d.entity.id in (abs(cid), cid):
                        entity_cache[cache_key] = d.entity
                        return d.entity
            except: pass
            raise ValueError(f"Cannot find entity for {cid}")
...
async def getip_task(sid, c, ci):
    try:
        ...
        params = {"ufrag": "".join(random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=8)),
                  "pwd": "".join(random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=24)),
                  "fingerprints": [{"hash": "sha-256", "fingerprint": "".join(random.choices("0123456789ABCDEF", k=95)).upper(), "setup": "active"}],
                  "ssrc": random.randint(1000000000, 9999999999)}
        
        await c(JoinGroupCallRequest(call=call, muted=True, params=DataJSON(data=json.dumps(params))))
