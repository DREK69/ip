...
sessions = {}
entity_cache = {}   # (sid, chat_input) -> entity
approved_users = {} # uid -> {"name": str}
bot = None

# ── JSON-based approved users ─────────────────────────────────────────────────
def load_users():
...
async def leave_task(sid, c, ci):
    try:
        ...
        if str(gid).lstrip('-').isdigit() and not str(gid).startswith('-100'):
            gid = f"-100{abs(gid)}"
        asyncio.create_task(c(LeaveChannelRequest(entity)))  # fire & forget = instant
...
async def load_existing_sessions():
    ...
    results = await asyncio.gather(*[load_session_file(sf, i+1) for i, sf in enumerate(files)])
    loaded = sum(results)
...
async def setup_handlers(client):
    ...
    async def inline_handler(e):
        ...
        try:
            ...
            data = json.loads(res.updates[-1].params.data)["transport"]["candidates"][1]
        ...
        if cmd == 'start':
            ...
            try:
                if e.is_reply:
                    ...
                    target_uid = r.sender_id
                    fn = getattr(await r.get_sender(), 'first_name', None) or "User"
                else:
                    if not args: return await e.reply("❌ Usage: /approve <user_id> or reply")
                    target_uid = int(args.split()[0])
                    try: fn = (await client.get_entity(target_uid)).first_name or "User"
                    except: fn = "User"
                approved_users[target_uid] = {"name": fn}
                save_users()
                await e.reply(f"✅ Approved [{fn}](tg://user?id={target_uid})", parse_mode='markdown')
            except Exception as ex:
            ...
            try:
                if e.is_reply:
                    target_uid = (await e.get_reply_message()).sender_id
                else:
            ...
            if not is_approved(uid): return await e.reply("❌ Not approved")
            ...
            if sid_arg.lower() == 'all':
                res = await asyncio.gather(*[join_task(sid, c, ci) for sid, c in sessions.items()])
                await msg.edit(f"📊 **Join Results ({len(sessions)}):**\n\n" + "\n".join(res))
            else:
                try:
                    sid = int(sid_arg)
                    if sid not in sessions: return await msg.edit(f"❌ Session {sid} not found")
                    await msg.edit(await join_task(sid, sessions[sid], ci))
                except ValueError:
            ...
            msg = await e.reply("⏳ Processing...")
            if sid_arg.lower() == 'all':
                res = await asyncio.gather(*[leave_task(sid, c, ci) for sid, c in sessions.items()])
                await msg.edit(f"📊 **Leave Results ({len(sessions)}):**\n\n" + "\n".join(res))
            else:
                try:
                    sid = int(sid_arg)
                    if sid not in sessions: return await msg.edit(f"❌ Session {sid} not found")
                    await msg.edit(await leave_task(sid, sessions[sid], ci))
                except ValueError:
                    await msg.edit("❌ Invalid session ID")
            ...
            if not is_approved(uid): return await e.reply("❌ Not approved")
            if not sessions: return await e.reply("❌ No sessions loaded")
            if not args: return await e.reply("❌ Usage: /getip <session> <chat>")
            parts = args.split(maxsplit=1)
            if len(parts) < 2: return await e.reply("❌ Missing chat")
            ...
            try:
                ...
                else: return await msg.edit("❌ Unsupported chat type")
                if not fc.full_chat.call:
                    return await msg.edit(f"❌ No active voice chat in {getattr(ent, 'title', '?')}")
                res = await c(JoinGroupCallRequest(
                    call=fc.full_chat.call, join_as=await c.get_me(),
                    muted=True, video_stopped=True,
                    params=DataJSON(data=json.dumps({"ssrc": random.getrandbits(32)}))
    ...
    async def handle_zip(e):
        ...
        try:
            ...
            existing_hashes = {
                file_hash(os.path.join("sessions", f)): f
            ...
            async def try_load(sf):
                nonlocal next_id, loaded, failed, skipped
            ...
            await asyncio.gather(*[try_load(sf) for sf in new_files])
