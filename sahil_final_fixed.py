                    ...
                    for sid, c in sessions.items():
                        try:
                            ...
                            if isinstance(ent, Channel):
                                ...
                                errors.append(f"S{sid}: Unsupported chat")
                                continue
                            
                            if not fc.full_chat.call:
                                errors.append(f"S{sid}: No voice chat")
                                continue
                            ...
                            errors.append(f"S{sid}: {str(ex)[:30]}")
                            continue
                    
                    if not success:
                        error_msg = "❌ **All sessions failed**\n\n" + "\n".join(errors[:10])
                        await e.reply(error_msg)
